  ...
  return axios.get('https://ecommerce.routemisr.com/api/v1/products')
    .then(({ data }) => {
      const filtered = data.data.filter(
        (item) =>
          /(^|\W)electronics(\W|$)/.test(item.category.slug.toLowerCase()) ||
          /(^|\W)electronics(\W|$)/.test(item.category.name.toLowerCase())
      );
  ...
  <div className="flex relative">
    ...
    {showDropdown && (
      <div id="dropdown" className="z-20 bg-white divide-y divide-gray-100 rounded-lg shadow-sm w-44 dark:bg-gray-700 absolute left-0 top-12">
        ...
        <li>
              <Link to={`/mencategory/men`}> <button type="button" className="inline-flex w-full px-4 py-2 hover:bg-gray-100 dark:hover:bg-gray-600 dark:hover:text-white">men's Fashion</button></Link>
          </li>
        </ul>
      </div>
    )}
